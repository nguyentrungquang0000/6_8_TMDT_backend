...
public class UserController {
    ...
    @PostMapping("/admin")
    public ResponseEntity<ResponseDto<UserResponse>> createAdmin(@RequestBody UserCreateRequest request) {
        UserResponse response = userService.createAdmin(request, Role.ADMIN);
        return ResponseBuilder.success(response, ResponseCode.SUCCESS);
    }
    ...
    @GetMapping
    public ResponseEntity<ResponseDto<List<UserResponse>>> search(@ModelAttribute UserSearch request) {
        Page<UserResponse> response = userService.search(request);
        MetaData metaData = MetaData.builder()
                .pageSize(request.size())
                .currentPage(request.page())
                .totalPage(response.getTotalPages())
                .build();
        return ResponseBuilder.success(response.getContent(), ResponseCode.SUCCESS, metaData);
    }
